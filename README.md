## Hola Me llamo TOMAS y este es mi GitHub
### Perfil
>Python (Básico) | | C# (Básico) | | IA: Gemini | | Gusto por Juegos Análisis y seguimiento de Sistemas Estudiante de Tecnicatura Universitaria en Programación

<div style="background-color: #1e1e1e; padding: 15px; display: inline-block; border-radius: 8px;">
  <a href="https://www.python.org">
    <img height="60" src="https://www.vectorlogo.zone/logos/python/python-ar21.svg" alt="Python Logo">
  </a>
</div>

<div style="background-color: #1e1e1e; padding: 15px; display: inline-block; border-radius: 8px;">
  <a href="https://learn.microsoft.com/dotnet/csharp/">
    <img height="60" src="https://upload.wikimedia.org/wikipedia/commons/4/4f/Csharp_Logo.png" alt="C# Logo">
  </a>
</div>

<!-- JavaScript, HTML5 y CSS en línea con 30px de separación entre sí -->
<div style="background-color: #1e1e1e; padding: 15px; display: inline-block; border-radius: 8px; margin-right: 30px;">
  <a href="https://developer.mozilla.org/es/docs/Web/JavaScript/">
    <img height="60" src="https://www.vectorlogo.zone/logos/javascript/javascript-ar21.svg">
  </a>
</div>

<div style="background-color: #1e1e1e; padding: 15px; display: inline-block; border-radius: 8px; margin-right: 30px;">
  <a href="https://es.wikipedia.org/wiki/HTML5/">
    <img height="60" src="https://www.vectorlogo.zone/logos/w3_html5/w3_html5-ar21.svg">
  </a>
</div>

<div style="background-color: #1e1e1e; padding: 15px; display: inline-block; border-radius: 8px; margin-right: 30px;">
  <a href="https://es.wikipedia.org/wiki/CSS">
    <img height="60" src="https://www.vectorlogo.zone/logos/w3_css/w3_css-ar21~old.svg">
  </a>
</div>

<div style="background-color: #1e1e1e; padding: 15px; display: inline-block; border-radius: 8px;">
  <a href="https://getbootstrap.com/">
    <img height="60" src="https://www.vectorlogo.zone/logos/getbootstrap/getbootstrap-ar21.svg">
  </a>
</div>

<div style="background-color: #1e1e1e; padding: 15px; display: inline-block; border-radius: 8px;">
  <a href="https://gemini.google.com/">
    <img height="60" src="https://upload.wikimedia.org/wikipedia/commons/8/8a/Google_Gemini_logo.svg" alt="Google Gemini Logo">
  </a>
</div>


